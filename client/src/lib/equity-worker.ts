// Optimized equity calculator matching original evaluator logic
...
// Match original: 2=2, 3=3, ..., T=10, J=11, Q=12, K=13, A=14
...
// Evaluate 5 cards using rank values 2-14 and suit indices 0-3
function eval5(r0: number, r1: number, r2: number, r3: number, r4: number,
  ...
  // Find quads, trips, pairs (sorted high to low)
  let quads = -1, trips = -1;
  const pairs: number[] = [];
  ...
  if (isFlush && straightHigh !== null) {
    if (straightHigh === 14) return 9 * BASE + straightHigh; // Royal
    return 8 * BASE + straightHigh; // Straight Flush
  }
  ...
  // Flush
  if (isFlush) {
    // Use base-15 encoding for 5 kickers
  ...
  // High Card
  return 0 * BASE + sorted[0] * 50625 + sorted[1] * 3375 + sorted[2] * 225 + sorted[3] * 15 + sorted[4];
}
...
function calculateEquity(players: PlayerInput[], board: Card[]): CalculationResult {
  ...
  for (const rank of RANKS) {
    for (const suit of SUITS) {
      if (!used.has(`${rank}${suit}`)) {
        deckR.push(RANK_VALUES[rank]);
        deckS.push(SUIT_MAP[suit]);
      }
    }
  }
  ...
  let processed = 0;
  const progressInterval = Math.max(1, Math.floor(totalTrials / 50));
  
  function evalOmaha(pIdx: number, br: number[], bs: number[]): number {
    const hr = playerRanks[pIdx], hs = playerSuits[pIdx];
    const hLen = hr.length;
    let best = 0;
    
    for (let hi = 0; hi < H2.length; hi++) {
      const [h0, h1] = H2[hi];
      if (h0 >= hLen || h1 >= hLen) continue;
      
      for (let bi = 0; bi < 10; bi++) {
        const [b0, b1, b2] = B3[bi];
        const score = eval5(hr[h0], hr[h1], br[b0], br[b1], br[b2],
  ...
  function updateWinners(scores: number[]) {
    let maxScore = 0;
    ...
    let winnerCount = 0;
    for (let p = 0; p < numPlayers; p++) {
      if (scores[p] === maxScore) winnerCount++;
    }
    for (let p = 0; p < numPlayers; p++) {
      if (scores[p] === maxScore) {
        if (winnerCount === 1) wins[p]++;
        else ties[p]++;
      }
    }
  }
  ...
  if (cardsNeeded === 0) {
    ...
    const fullR = [...boardR, 0, 0];
    const fullS = [...boardS, 0, 0];
    
    for (let i = 0; i < deckLen - 1; i++) {
      fullR[3] = deckR[i]; fullS[3] = deckS[i];
      for (let j = i + 1; j < deckLen; j++) {
        fullR[4] = deckR[j]; fullS[4] = deckS[j];
        
        for (let p = 0; p < numPlayers; p++) scores[p] = evalOmaha(p, fullR, fullS);
        updateWinners(scores);
    ...
    const fullR = [0, 0, 0, 0, 0];
    const fullS = [0, 0, 0, 0, 0];
    
    for (let a = 0; a < deckLen - 4; a++) {
      fullR[0] = deckR[a]; fullS[0] = deckS[a];
      for (let b = a + 1; b < deckLen - 3; b++) {
        fullR[1] = deckR[b]; fullS[1] = deckS[b];
        for (let c = b + 1; c < deckLen - 2; c++) {
          fullR[2] = deckR[c]; fullS[2] = deckS[c];
          for (let d = c + 1; d < deckLen - 1; d++) {
            fullR[3] = deckR[d]; fullS[3] = deckS[d];
            for (let e = d + 1; e < deckLen; e++) {
              fullR[4] = deckR[e]; fullS[4] = deckS[e];
    ...
    for (let i = 0; i < cardsNeeded; i++) indices.push(i);
    
    const fullR = [...boardR];
    const fullS = [...boardS];
    for (let i = 0; i < cardsNeeded; i++) { fullR.push(0); fullS.push(0); }
    
    while (true) {
      for (let i = 0; i < cardsNeeded; i++) {
        fullR[board.length + i] = deckR[indices[i]];
        fullS[board.length + i] = deckS[indices[i]];
      }
      ...
      let i = cardsNeeded - 1;
      while (i >= 0 && indices[i] === deckLen - cardsNeeded + i) i--;
      if (i < 0) break;
      indices[i]++;
      for (let j = i + 1; j < cardsNeeded; j++) indices[j] = indices[j - 1] + 1;
    }
  }
  
  const equityResults: EquityResult[] = validPlayers.map((p, idx) => ({
    playerId: p.id,
    wins: wins[idx],
    ties: ties[idx],
    total: processed,
    equity: processed > 0 ? ((wins[idx] + ties[idx] / 2) / processed) * 100 : 0
  }));
  
  return { results: equityResults, totalTrials, isExhaustive: true };
}
